...
- 🟡 **部分满足**：通用交互细节（小屏/大屏适配）、历史视觉细节与数据口径一致性
- ❌ **未满足**：历史查询 N+1 与统计一致性验收、全链路真机回归

> 2026-03-05 更新：目标页已切换为高密度清单样式；设置页已落地本地备份恢复与通知开关联动；P1 路由动效与专注页小屏可达性完成。  
> 2026-03-05（晚）更新：关键列表页 Header 显隐已统一（`NestedScrollView + SliverAppBar`），并新增 `LayoutTokens` 收口跨页间距节奏。
...
- ✅ Header 滚动隐藏/显示：已实现统一方案（首页/目标/专注/历史/分析/我的/设置采用 `NestedScrollView + SliverAppBar(floating+snap)`；搜索态保持固定 `AppBar`）
...
- **P1 一致性问题**
  - [x] 各页面间距密度与卡片层级不完全一致（紧凑度不统一）
  - [x] Header 行为不统一（滚动页未实现显隐策略）
...
- [x] 在 Phase P1 增加统一的间距/组件密度规范（形成 UI tokens 或页面规范）

### 6.3 体验验收指标（前端向）

- [x] 360dp 宽度设备下，专注页无需滚动即可完成一次开始操作
- [ ] 历史时间轴首屏可一眼区分时间分组与记录归属
- [x] 关键页面（首页/目标/专注/历史）垂直节奏一致，无明显“忽松忽紧”
